...
void PacketPoolInit(void)
{
	...
	printf("\nmax_pending_packets :%ld default_packet_size:%d\n",max_pending_packets,default_packet_size);
    int i = 0;
...
void TmqhOutputPacketpool(ThreadVars *t, Packet *p)
{
    ...
    SCEnter();
    //printf("Packet %p, p->root %p, alloced %s", p, p->root, p->flags & PKT_ALLOC ? "true" : "false");

    if (IS_TUNNEL_PKT(p)) {
        printf("Packet %p is a tunnel packet: %s",
